...
set "QGIS_TEXT=QGIS DEV (serverbasiert, OSGeo4W)"
...
set "QGIS_UDIR=%OSGEO4W_ROOT%\qgis_dev_template"

REM Patch QGIS-EPSG-DB for German BeTA-2007-Transformation (GK2/3/4/5->UTM32/33)
"%OSGEO4W_ROOT%"\bin\sqlite3.exe "%OSGEO4W_ROOT%"\apps\qgis-dev\resources\srs.db < "%OSGEO4W_ROOT%"\apps\qgis-dev\resources\BETA2007.sql
...
set "UO=%PO%/qgis_dev_template"

echo off
REM Copy user dir template to the final path on user-pc 
xcopy "%PN%/qgis_dev_template" "%UN%" /e /i
...
if not exist "%ProgramData%\Microsoft\Windows\Start Menu\Programs\QGIS\%QGIS_TEXT%.lnk" (
   if not exist "%USERPROFILE%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\QGIS\%QGIS_TEXT%.lnk" (
      nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-dev-start.bat" "~$folder.programs$\QGIS" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
	  )
   )
...
if not exist "%PUBLIC%\Desktop\%QGIS_TEXT%.lnk" (
   if not exist "%USERPROFILE%\Desktop\%QGIS_TEXT%.lnk" (
      nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-dev-start.bat" "~$folder.desktop$" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
      )
   )

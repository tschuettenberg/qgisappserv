REM Batch-Datei for initialize QGIS DEV 
echo on
chcp 1252
...
if not exist "%ProgramData%\Microsoft\Windows\Start Menu\Programs\QGIS\%QGIS_TEXT%.lnk" (
   nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-dev-start.bat" "%ProgramData%\Microsoft\Windows\Start Menu\Programs\QGIS" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
   )
...
if not exist "%PUBLIC%\Desktop\%QGIS_TEXT%.lnk" (
   nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-dev-start.bat" "%PUBLIC%\Desktop" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
   )

REM Create file and icon associations 
start/wait regedit -s "%OSGEO4W_ROOT%\bin\qgis.dev.reg"

...
start "MSYS Shell" "%WD%mintty" /bin/bash -l
exit
...
start "MSYS Shell" "%WD%rxvt" -backspacekey  -sl 2500 -fg %FGCOLOR% -bg %BGCOLOR% -sr -fn Courier-12 -tn msys -geometry 80x25 -e /bin/sh --login -i
exit

:startsh
if NOT EXIST "%WD%sh.exe" goto notfound
start "MSYS Shell" "%WD%sh" --login -i
exit

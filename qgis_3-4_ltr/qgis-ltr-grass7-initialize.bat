REM Batch-Datei for initialize QGIS 3.4 LTR
echo on
chcp 1252
...
if not exist "%ProgramData%\Microsoft\Windows\Start Menu\Programs\QGIS\%QGIS_TEXT%.lnk" (
   nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-ltr-grass7-start.bat" "%ProgramData%\Microsoft\Windows\Start Menu\Programs\QGIS" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\apps\qgis-ltr\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
   )
...
if not exist "%PUBLIC%\Desktop\%QGIS_TEXT%.lnk" (
   nircmd shortcut """%OSGEO4W_ROOT%""\bin\qgis-ltr-grass7-start.bat" "%PUBLIC%\Desktop" "%QGIS_TEXT%" "" """%OSGEO4W_ROOT%""\apps\qgis-ltr\icons\QGIS.ico" "0" "min" """%OSGEO4W_ROOT%""\bin" ""
   )
